...
./Scripts/notarize-release.sh 1.0.2
```
...
4. Render the Homebrew Cask from the published DMG's SHA-256 and audit/install it against the public URL. For example, `Scripts/render-homebrew-cask.sh 1.0.2 HASH /path/to/drawstate.rb`. Obtain approval before pushing the cask to `Kian-hdr/homebrew-drawstate`.
